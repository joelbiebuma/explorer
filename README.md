...
By default it will look for blockchain in its default location i.e., `~/.txchangecoin/lmdb`.
...
Every 10000 blocks, the thread will save current emission in a file, by default,
 in `~/.txchangecoin/lmdb/emission_amount.txt`. For testnet or stagenet networks,
 it is `~/.txchangecoin/testnet/lmdb/emission_amount.txt` or `~/.txchangecoin/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the
 explorer restarts, the thread will first check if `~/.txchangecoin/lmdb/emission_amount.txt`
